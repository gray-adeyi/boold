...
export function drawBoardFrame() {
	...
	// Scroll animation
	if (
		state.settings.isScrollAnimationEnabled &&
		state.scrollAnimation.animate
	) {
		state.offset.x -=
			Math.sin(state.scrollAnimation.r) * state.scrollAnimation.v;
		state.offset.y +=
			Math.cos(state.scrollAnimation.r) * state.scrollAnimation.v;
	...
	if (state.settings.isZoomAnimationEnabled) {
		...
		state.offset.x =
			state.offset.x +
			state.mouse.screen.x * (1 / state.zoom - 1 / state.zoomAnimation);
		state.offset.y =
			state.offset.y -
			state.mouse.screen.y * (1 / state.zoom - 1 / state.zoomAnimation);
