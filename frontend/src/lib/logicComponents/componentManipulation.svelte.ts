...
export function addComponent(
  ...
  if (
    (!findComponentPinByPos(x, y, boardStoreState) && !findWireByPos(x, y, boardStoreState)) ||
    force
  ) {
...
export function removeWire(wire: Wire, boardStoreState: BoardStoreState): Wire[] {
...
export function moveComponent(
  ...
  const oldPos: Coord = Object.assign(
    {},
    boardStoreState.userDrag && boardStoreState.userDrag?.component === component
  ...
  if (pin.type === "input") {
    ...
    const vertical = () => {
      for (let i = 0; i < Math.abs(dy); i++) {
        ...
        const index = wire.path.findIndex((coord) => coord.x === x && coord.y === y);
        if (index > -1) {
    ...
    for (let i = 0; i < component.inputPins.length; i++) {
      ...
      if (wire && !wires.includes(wire)) {
        ...
        const index = wire.path.findIndex(
          (coord) => coord.x === wire.path.slice(-1)[0].x && coord.y === wire.path.slice(-1)[0].y,
        );
        if (index > -1) {
          wire.path.splice(index + 1, wire.path.length);
          continue;
...
export function editEntity(entity: BoardEntity, property: string, value: unknown) {
  if (Object.hasOwn(entity, property) && typeof value === typeof entity[property]) {
...
export function findComponentByName(
  ...
  return boardStoreState.components.find((component) => component.name === name);
}
 ...
 *  undefined
 */
export function findWireById(id: string, boardStoreState: BoardStoreState): Wire | undefined {
...
export function findComponentPinInComponent(
  ...
  return component.inputPins.find(predicateFn) || component.outputPins.find(predicateFn);
}
...
export function findComponentPinByPos(
  ...
  for (let i = 0; i < 4; i++) {
    ...
    if (component) {
      const side = i;
      const sideIndex = side % 2 === 0 ? x - component.pos.x : component.pos.y - y;
...
export function findWiresInUserSelection(
  ...
  for (let i = 0; i < boardStoreState.wires.length; i++) {
    ...
    for (let j = 0; j < path.length; j++) {
      if (path[j].x >= x && path[j].x <= x2 && path[j].y >= y && path[j].y <= y2) {
...
export function findAllWiresInSelectionWithoutConnections(
  ...
  for (let i = 0; i < result.length; i++) {
    ...
    if (wire?.from?.component && !boardStoreState.components.includes(wire.from.component)) {
      result.splice(i, 1);
      i = -1;
      continue;
    }

    if (wire?.to?.component && !boardStoreState.components.includes(wire.to.component)) {
      result.splice(i, 1);
      i = -1;
      continue;
    }
...
export function cloneComponent(
  ...
  if (clone.name.includes(`${clone.constructor.name}#`)) {
    ...
    const baseName = clone.name.includes("#") ? clone.name.split("#")[0] : clone.name;
...
export function cloneWire(
  ...
  const newIntersections: WireIntersection[] = wire.intersections.map((intersection) => {
    return {
...
export function componentize(
  ...
  const clone = cloneSelection(components, wires, undefined, undefined, boardStoreState);
  ...
  removeSelection(userSelection.components, userSelection.wires, boardStoreState);
  boardStoreState.components.push(component);
}
