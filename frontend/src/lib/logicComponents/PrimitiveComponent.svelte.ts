...
export default class PrimitiveComponent implements LogicComponent {
  ...
  rotate() {
    ...
    for (let i = 0; i < this.inputPins.length; i++) {
      this.inputPins[i].placement.side = ++this.inputPins[i].placement.side % 4;
    }

    for (let i = 0; i < this.outputPins.length; i++) {
      this.outputPins[i].placement.side = ++this.outputPins[i].placement.side % 4;
    }
  }
  ...
  protected drawPins(
    ...
    for (let i = 0; i < pins.length; i++) {
      ...
      ctx.beginPath();
      ctx.moveTo(screen.x - (Math.sin(angle) / 2) * zoom, screen.y + (Math.cos(angle) / 2) * zoom);
